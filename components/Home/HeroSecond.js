...
export default function HeroSecond() {
  ...
  return (
    <section className='flex flex-col -top-28 md:-top-12 lg:top-0 lg:flex-row relative items-center justify-between mb-0 lg:mb-96 w-full' id='hero2'>
        <PolysShot className="absolute rotate-180 -z-0 -top-80 -right-44 lg:block hidden"/>
        <Octagons className="absolute lg:left-1/2 lg:-translate-x-1/3  lg:translate-y-full translate-x-44 translate-y-2/3" />
        <div className="flex relative h-[361px] md:h-[418px] justify-center md:w-[295px] lg:w-[590px] items-center z-0 md:z-10">
            ...
            <div id="hero-img" className='relative flex justify-center items-center h-full w-full'>
                    ...
                    <div ref={wolfRef} className="absolute translate-x-[11px] -translate-y-[22px] md:translate-x-[15px] md:-translate-y-[33px] lg:translate-x-[24px] lg:-translate-y-[61px]">
                        <Image src='/home/wolfHero.svg' width={512} height={498} className={`relative h-[189px] w-[177px] md:h-[254px] md:w-[256px] lg:h-[498px] lg:w-[512px]  transition-all duration-1000 ease-out ${sectionVisible ? 'translate-x-0' : 'translate-x-14'}`}/>
                    </div>
                </div>
            </div>
        </div>
        <div className='flex relative flex-col text-second-heavy mx-2 lg:mx-0 gap-3 z-10 max-w-[900px] w-full mt-6 lg:mt-12'>
